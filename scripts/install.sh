...
VERSION="${CHATWOOT_CLI_VERSION:-latest}"
...
curl -fsSL "$asset_url" -o "$TMP_DIR/chatwoot-cli.tar.gz"
tar -xzf "$TMP_DIR/chatwoot-cli.tar.gz" -C "$TMP_DIR"

BIN_DIR="${BIN_DIR:-$HOME/.local/bin}"
mkdir -p "$BIN_DIR"
install -m 755 "$TMP_DIR/chatwoot" "$BIN_DIR/chatwoot"

echo "installed: $BIN_DIR/chatwoot"
